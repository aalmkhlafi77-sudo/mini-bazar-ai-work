...
import { CheckoutMapPicker, SelectedLocationData } from './CheckoutMapPicker';
...
export const CheckoutView: React.FC = () => {
  ...
  // Geolocation & Map picking states
  const [addressInputMethod, setAddressInputMethod] = useState<'map' | 'manual'>('map');
  ...
  const handleLocationSelected = (data: SelectedLocationData) => {
    ...
    setAutofilledNotice('تمت تعبئة حقول العنوان تلقائياً وفقاً للموقع المحدد.');
    setTimeout(() => {
    ...
    <div className="py-10 px-4 sm:px-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <form onSubmit={handleSubmitOrder} className="lg:col-span-7 space-y-6 text-right">
          ...
          {/* 2. Delivery Address with Geolocation & Interactive Map */}
          <div className="bg-white p-6 rounded-[24px] border border-[#E5D8C9] shadow-2xs space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#F4ECE2] pb-3.5">
              <h3 className="text-base font-bold text-[#6F584A] font-heading flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-[#F4ECE2] text-[#C6A36A] flex items-center justify-center text-xs">
                  2
                </span>
                <span>عنوان التوصيل وتحديد الموقع</span>
              </h3>

              {/* Mode Switcher Tabs */}
              <div className="flex items-center p-1 rounded-[12px] bg-[#F4ECE2] border border-[#E5D8C9]">
                <button
                  type="button"
                  onClick={() => setAddressInputMethod('map')}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-[9px] text-xs font-semibold transition-all ${
                    addressInputMethod === 'map'
                      ? 'bg-white text-[#2F2B28] shadow-xs'
                      : 'text-[#7C736D] hover:text-[#2F2B28]'
                  }`}
                >
                  <MapPin className="w-3.5 h-3.5 text-[#C6A36A]" />
                  <span>الخريطة و GPS التلقائي</span>
                </button>
                <button
                  type="button"
            ...
            {addressInputMethod === 'map' && (
              <div className="pt-1">
                <CheckoutMapPicker
                  initialLat={latitude || 24.7136}
                  initialLng={longitude || 46.6753}
                  selectedCity={city}
            ...
            {autofilledNotice && (
              <div className="p-3 rounded-[12px] bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs flex items-center gap-2">
            ...
            <div className="pt-2">
              <span className="block text-[11px] font-semibold text-[#8A7465] mb-2.5">
                بيانات العنوان (يتم تعبئتها تلقائياً ويمكنك تعديلها يدوياً):
              </span>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-[#5F5751] mb-1.5">
